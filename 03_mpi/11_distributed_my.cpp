...
int main(int argc, char** argv) {
  ...
  Body ibody[N/size], jbody[N/size], jbody_temp[N/size];
  srand48(rank);
  ...
  for(int irank=0; irank<size; irank++) {
    ...
    MPI_Win_create(jbody_temp, N*sizeof(Body)/size, sizeof(Body), MPI_INFO_NULL, MPI_COMM_WORLD, &win);
    MPI_Win_fence(0, win);
    MPI_Put(jbody, N/size, MPI_BODY, send_to, 0, N/size, MPI_BODY, win);
    MPI_Win_fence(0, win);
    for(int i=0; i<N/size; i++) {
        jbody[i]=jbody_temp[i];
    }
    for(int i=0; i<N/size; i++) {
